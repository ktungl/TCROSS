...
| Phase 1 — Cloud Run 中介層 | ✅ **已部署（含今天新增的 `/download-url`）** | GCP 專案 `project-80ac5e1a-2ea4-4000-9ff`（Tcross，billing 已啟用），服務 `tcross-middleware` 跑在 `asia-east1`。2026-08-20 用 `gcloud run deploy` 重新部署（revision `tcross-middleware-00004-zdh`），`GET /status` 與 `POST /download-url`（含 `is_valid_object_path()` 路徑檢查）都已在線上實測通過，環境變數/secrets/服務帳戶皆沿用原設定未變動。 |
